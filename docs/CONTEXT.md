...
> 最終更新: 2026-04-19 | v6.42 | 売れる目標設定・Enemy Design設計完了
...
| **主婦 (Housewife)** | UI/UX・操作性検討時 | 隙間時間の遊びやすさ・直感性 |
| **学生 (Student)** | 演出・ビジュアル検討時 | トレンド・エモさ・SNS映え |
| **ゲーマー (Gamer)** | 全般（特にバランス・QoL） | 操作レスポンス・攻略の深さ |
...
- 2026-03-28: Pythonスクリプト生成はヒアドキュメント + Set-Content経由で.pyファイル保存後に実行する形式に統一。PowerShellの引用符干渉回避のため。インラインpython -c は使わない。


- 2026-03-28: GitHub MCP連携を使いファイル取得はツールで行う（Raw URL手動貼り付け不要）
- 2026-03-28: GitHub MCP連携を使いファイル取得はツールで行う（Raw URL手動貼り付け不要）
- 2026-03-28: タブUIキーヒント表示タスクを次セッションへ持ち越し（原神風）
- 2026-03-28: コード変更はPythonスクリプト生成（Set-Content→python実行）、確認後に次ステップへ進むルールを明文化
- 2026-03-28: セッションプロトコルに「必要ファイルのRaw URLを列挙→ユーザーに貼付」ルール追加
- 2026-03-28: セッション終了。次タスク：コレクションUI統一リファクタ（設計書あり）
- 2026-03-28: いきもの図鑑をトレカ型グリッド表示に変更
- 2026-03-28: 図鑑カードloreテキストはみ出し修正
...
バージョン: v6.40 → v6.41
最終更新: 2026-03-29
テスト: 159/159 PASS（変わらず）
ルール変更履歴に追記:

2026-03-29: タッチ座標ズレ修正・ヘルプUI改善・HUD重なり解消
