...
文書ID: STATUS-v5.0 | 更新日: 2026-03-04 (v6.0.5)
...
| Sprint | 内容 | ポイント | 状態 |
|---|---|---|---|
| Sprint 1 | ヒットストップ + SE統合 | +4 | ✅ 完了 (v5.3.0) |
| Sprint 2 | マップ多様性 | +2 | ✅ 完了 (v5.4.0) |
| Sprint 3 | ビルド多様性 | +2 | ✅ 完了 (v5.5.0) |
| Sprint 4 | ビジュアル/スプライト | +3 | 🔧 敵12+ボス4完了, 武器/アイテム11枚残 |
| Sprint 5 | メタ基盤 | +2 | ✅ 完了 (v5.6.0) |
| Sprint 5.5 | 装備/消耗品 | — | ✅ 完了 (v5.7.0) |

### 追加実装 (Sprint外)

| 機能 | 状態 |
|---|---|
| 統一メッセージシステム (float/dialog) | ✅ v5.8.0 |
| スプライトエンジン (4シート対応, フォールバック) | ✅ v5.9.0 |
| ショップUI刷新 | ✅ v5.7.1 |
| UX改善 (操作案内, チュートリアル) | ✅ v5.7.2 |
| ゲームオーバー演出改善 | ✅ v5.6.1 |
...
| 課題 | 優先度 | 備考 |
|---|---|---|
| Sprint 4: 残りスプライト11枚 | 高 | 武器6+消耗品3+ドロップ2, 手動生成+rembg処理 |
| Geminiバッチプリセット更新 | 低 | 手動生成に切替済, バッチ不要 |
| ミプリン歩行アニメーション | 高 | 4方向×3フレーム, 専用スプライトシート |
| drawEnemyShape内の重複mipurinReady分岐 | 低 | v5.9.0で解消済み |
| 未使用jsファイル30+個の削除 | 低 | Phase 1完了後に実施 |
| player_sheet.png/player.json 未使用 | 低 | 新スプライトシート導入後に整理 |
| STATUS-v5.0.md Sprint 1-3チェックボックス | 低 | 本パッチで表形式に変更 |
| 開発ルール「忖度禁止」の運用 | — | 継続監視 |

### 現在のスコア推定 (v5.9.1)

| カテゴリ | v5.2.1 | v5.9.1 | Phase1目標 |
|---|---|---|---|
| コアループ | 5.5 | 6.5 | 7 |
| メタ進行 | 3 | 5 | 5 |
| ランダム性 | 4 | 5 | 5.5 |
...
| v5.3.0 | e0ffec2 | Sprint 1完了: ヒットストップ, SE接続(15mp3), ノックバック, パーティクル強化 |
| v5.4.0 | — | Sprint 2: 5種部屋テンプレート(テーマ連動), flood-fill経路保証, トゲ床(F4+) |
| v5.4.2 | 9f868a6 | Hotfix: 黄色背景除去, 武器range+12, ノックバック32px, needle dur修正 |
| v5.4.3 | — | 攻撃エフェクト改善: 黄色矩形→スラッシュ弧(暗輪郭+白フラッシュ) |
| v5.5.0 | — | Sprint 3: 祝福24種, デュオ6種, エンディング(F15), rarity重み修正 |
| v5.5.2 | — | 攻撃エフェクト視認性改善: 暗輪郭+白弧+武器色弧 |
| v5.6.0 | — | Sprint 5: ネクター通貨, 花壇アップグレード(HP/ATK), localStorage永続化 |
| v5.6.1 | — | ゲームオーバー演出改善: 2秒ガード, スコア表示, 点滅プロンプト |
| v5.7.0 | — | Sprint 5.5: 武器2本持ち(おきにいり/もうひとつ), 消耗品3種, 武器コレクション |
| v5.7.1 | — | ショップUI刷新: 2行×3列グリッド, 日本語テキスト, WASD操作 |
| v5.7.2 | — | UX改善: ショップX脱出, 操作案内バー, アイテムスロットラベル, チュートリアルメッセージ |
| v5.7.3 | — | メッセージ表示改善: 表示時間延長(2s), 上部中央配置, 金枠 |
| v5.8.0 | c5f9653 | 統一メッセージシステム: float/dialog型, ボス台詞, フロア開始演出, テーマ名 |
| v5.8.1 | f7ce774 | メッセージ描画順修正: drawFloatMessages/drawDialogWindowをdraw()最後に移動 |
| v5.8.2 | b672455 | dialog状態ハンドラ追加: Zキーでダイアログ送り |
| v5.9.0 | 17bd883 | スプライトエンジン: シートローダー, drawEntity/drawBoss/drawDropsスプライト対応, canvasフォールバック |
| v5.9.1 | — | HUD文字かぶり修正, STATUS更新 |
| v5.9.2 | — | スプライトエンジン: 個別PNG方式, コードアニメーション(bob/squash), 背景除去ツール |
| v6.0.0 | 57a8ce3 | スプライト統合: 敵12種+ボス4種 WebP, SPRITE_MAP, 背景除去パイプライン |
| v6.0.1 | 89491a4 | スプライトエイリアス修正 (blob→slime) |
| v6.0.2 | dfa3c17 | スプライト描画アスペクト比維持, 20%スケールアップ |
| v6.0.3 | a7bbf33 | ゲームオーバー文字重なり修正 (旧英語テキスト削除) |
| v6.0.4 | 812b0ed | 敵スプライトアニメーション: bob + squash/stretch + tilt |
| v6.0.5 | 30ed230 | ゲームオーバー後BGM残留修正, アニメーション強化 |
