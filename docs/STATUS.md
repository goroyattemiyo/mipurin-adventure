...
※ コアループ+0.3(→8.3): 環境ギミック(水場/草むら/爆発樽)でダンジョン探索の多様性向上
※ ランダム性+0.2(→6.2): テーマ別ギミック配置でフロアごとに異なる戦術判断が必要
