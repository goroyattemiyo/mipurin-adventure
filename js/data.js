...
function fadeOutBGM(duration, onDone) {
  if (!bgmAudio) { if (onDone) onDone(); return; }
  const step = 0.05, steps = Math.max(1, duration / step);
  const volDec = bgmAudio.volume / steps;
  clearInterval(bgmFadeInterval);
  bgmFadeInterval = setInterval(() => {
    if (!bgmAudio) { clearInterval(bgmFadeInterval); if (onDone) onDone(); return; }
    bgmAudio.volume = Math.max(0, bgmAudio.volume - volDec);
    if (bgmAudio.volume <= 0.001) {
      clearInterval(bgmFadeInterval); bgmAudio.pause(); bgmAudio.currentTime = 0; bgmAudio = null; currentBGM = '';
      if (onDone) onDone();
    }
  }, step * 1000);
}
function playBGM(name, fadeIn) {
  if (currentBGM === name) return;
  const start = () => {
    currentBGM = name;
    bgmAudio = new window.Audio('assets/music/' + name + '.mp3');
    bgmAudio.loop = true;
    const target = (typeof bgmVolume !== 'undefined' ? bgmVolume : 0.7) * 0.3;
    if (fadeIn) {
      bgmAudio.volume = 0;
      bgmAudio.play().catch(() => {});
      const fi = fadeIn, step = 0.05, volInc = target / Math.max(1, fi / step);
      const iv = setInterval(() => {
        if (!bgmAudio) { clearInterval(iv); return; }
        bgmAudio.volume = Math.min(target, bgmAudio.volume + volInc);
        if (bgmAudio.volume >= target - 0.001) { bgmAudio.volume = target; clearInterval(iv); }
      }, step * 1000);
    } else {
  ...
  if (bgmAudio) { fadeOutBGM(0.8, start); } else { start(); }
}
function stopBGM(fadeDur, onDone) {
  if (fadeDur) { fadeOutBGM(fadeDur, onDone); }
  else { clearInterval(bgmFadeInterval); if (bgmAudio) { bgmAudio.pause(); bgmAudio.currentTime = 0; bgmAudio = null; } currentBGM = ''; if (onDone) onDone(); }
}
